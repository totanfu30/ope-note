...
- **並び替え**: 登録No.（昇順/降順）・手術日（新しい順/古い順）
- **絞り込み**: ID・疾患名・執刀医の項目別の欄。複数指定でAND絞り込み
- **カウント番号・総数は絞り込み結果に連動**: 絞り込んだ集合の中で手術日昇順の順位を「カウント n / 総数」として表示（例: 執刀医で絞ればその術者の通算件数と何例目かが分かる）。登録No.は絞り込みに関係なく不変
